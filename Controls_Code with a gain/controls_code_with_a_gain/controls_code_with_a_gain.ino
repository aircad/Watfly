...
//Gains
pGain=0.9;//.8 before
dGain=0.005;//.25 before
aGain=0.0;
iGain=0.1; //0.1 before
loopTime = millis();
...
{       
        ...
        if(fabs(RotVelocity)>5000)
        
        {
          RotVelocity = RotVelocity/fabs(RotVelocity)*5000;
        }
        
        if(fabs(RotAccel)>25000)

        {
          RotAccel = RotAccel/fabs(RotAccel)*25000;
        }

       
        aGainCalc=aGain*(RotVelocity/fabs(RotVelocity));
        ...
        */
        ...
        RightRpm=1250-(Angle*pGain)-(avgVel*dGain)-(RotAccel*aGainCalc)-angInt*iGain;  //Edited  changed rot velocity to -
        LeftRpm=1250+(Angle*pGain)+(avgVel*dGain)+(RotAccel*aGainCalc)+angInt*iGain;    //Edited
        
        if (RightRpm>1800) RightRpm=1800;
        if (LeftRpm>1800) LeftRpm=1800;
